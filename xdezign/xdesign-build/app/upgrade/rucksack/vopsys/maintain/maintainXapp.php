...
class maintainXapp extends maintainOffice{
    ...
    function fn_maintainMetaRowz($obj_metaView){
      ...
      `FormOrder`=
      CASE `MetaColumnName`
      WHEN 'DebugPin' THEN 0      
      WHEN 'MetaRowzName' THEN 15
      WHEN 'MetaRowzTitle' THEN 20
      WHEN 'MetaPermissionTag' THEN 30      
      WHEN 'RowzOrder' THEN 50
      WHEN 'RowzIcon' THEN 55
      WHEN 'HiddenPin' THEN 60
      WHEN 'Subdomain' THEN 70      
      ELSE 1000
      END
      WHERE TRUE 
      AND `MetaColumnSystemId`=$obj_param->MetaViewSystemId
      ...
      AND `MetaTableName`='meta_rowz'                        
      ;";                    
      //AND `MetaColumnName`IN ('DebugPin','MetaRowzName', 'MetaRowzTitle','MetaPermissionTag','RowzOrder','RowzIcon','HiddenPin','Subdomain')
      $stmt=$this->fn_executeSQLStatement($str_sql);
    }
    ...
    function fn_maintainMetaColumn($obj_param){
      ...
      OR `MetaColumnName`IN ('LivePin','MetaLabel','DebugPin','MetaType','MetaList','MetaOption','FormOrder','DefaultValue','Placeholder','MenuPin','MetaPermissionTag'))
      ;";                    
      $stmt=$this->fn_executeSQLStatement($str_sql);            
      ...
      CASE `MetaColumnName`
      WHEN 'LivePin' THEN 0
      WHEN 'MetaLabel' THEN 10
      WHEN 'DebugPin' THEN 20
      WHEN 'MetaType' THEN 30
      WHEN 'MetaType' THEN 40
      WHEN 'MetaList' THEN 50
      WHEN 'MetaOption' THEN 55      
      WHEN 'FormOrder' THEN 60
      WHEN 'DefaultValue' THEN 70
      WHEN 'Placeholder' THEN 80
      WHEN 'MenuPin' THEN 90
      WHEN 'MetaPermissionTag' THEN 100      
      ...
      AND `MetaTableName`='meta_column'                        
      ;";                    
      //AND `MetaColumnName`IN ('LivePin','MetaLabel','MetaType','MetaList','MetaOption','FormOrder','DefaultValue','Placeholder','MenuPin,'MetaPermissionTag')
      $stmt=$this->fn_executeSQLStatement($str_sql);
    }
