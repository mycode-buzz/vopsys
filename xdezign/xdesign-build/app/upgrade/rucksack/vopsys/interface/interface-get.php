...
class interface_get extends interface_search{
    ...
    function fn_interfaceGetMetaViewColumnSQL(){                        
        ...
        $str_sql.=" ORDER BY `meta_column`.`FormOrder`;";                       
    ...
    function fn_interfaceSelectFieldListColumnz(){        
        ...
        $str_fieldlist="                                    
            ...
            `meta_column`.`meta_column`.`MetaType`,
            ...
            `meta_column`.`meta_column`.`Decimal`,
            `meta_column`.`meta_column`.`UnSigned`,
            `meta_column`.`meta_column`.`MaxLength`,
            `meta_column`.`meta_column`.`RequiredPin`,            
            `meta_column`.`meta_column`.`DateTime`,            
            `meta_column`.`meta_column`.`DateTimeSecond`,            
            ...
            `meta_column`.`meta_column`.`PlaceHolder`,            
    ...
    function fn_interfaceCollateMetaViewColumns(){                                
        ...
        if(!$obj_paramView->ValidView){                                
            //$this->fn_addEcho("END fn_collateMetaViewColumns - valid view is false");                            
            return;
        }
        ...
        if($this->bln_debugRunSelect){
            $this->fn_addEcho("columns str_sql: ".$str_sql);                
            //$this->fn_addConsole("str_sql: ".$str_sql);
        }
        ...
        if(!$arr_rows && $obj_paramView->ValidView){                                            
            ...
            $obj_paramView->MetaUserSystemId=$this->obj_userLogin->MetaUserSystemId;            
            $this->fn_pushColumnzSystem($obj_paramView);//attempt to create                                                
    ...
    function fn_interfaceCollateMetaColumn(){        
        ...
        foreach ($arr_rows as $arr_row) {            
            ...
            $obj_meta->MetaType=$arr_row["MetaType"];
            ...
            $obj_meta->Decimal=$arr_row["Decimal"];
            $obj_meta->UnSigned=$arr_row["UnSigned"];            
            $obj_meta->MaxLength=$arr_row["MaxLength"];                        
            $obj_meta->RequiredPin=$arr_row["RequiredPin"];                        
            $obj_meta->DateTime=$arr_row["DateTime"];            
            $obj_meta->DateTimeSecond=$arr_row["DateTimeSecond"];            
            ...
            $obj_meta->PlaceHolder=$arr_row["PlaceHolder"];
